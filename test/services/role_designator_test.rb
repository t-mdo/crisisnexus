...
class RoleDesignatorTest < ActionDispatch::IntegrationTest
  setup do
    ...
    Account.all.each_with_index do |account, index|
      create(:role_enrollment, account:, role: Role.incident_manager, last_exercised_at: index.days.ago)
    end
  ...
  test 'designates incident manager in a organization' do
    last_exercised_account = RoleEnrollment.where(role: Role.incident_manager).order(last_exercised_at: :desc).first.account
    ...
    create(:role_enrollment, account: other_organization_account, role: Role.incident_manager, last_exercised_at: nil)
